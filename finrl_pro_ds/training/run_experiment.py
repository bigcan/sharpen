...
from finrl_pro_ds.mlops.risk_profiles import load_risk_profile
from finrl_pro_ds.training.trainer import Trainer
from finrl_pro_ds.envs.wrappers import TurnoverPenaltyWrapper, ActionSmoothingWrapper, SoftmaxAllocationWrapper
...
from finrl_pro_ds.utils.naming import generate_experiment_name
...
def _run_real_training(
    *,
    ...
    if tickers and 'tic' in df.columns:
        ...
        logger.log_event("finrl_pro_ds.training.tickers_filtered", context={"count": len(tickers)})
            
    if df.empty:
        raise ValueError(f"No data found for range {start_date} to {end_date}")

    logger.log_event("finrl_pro_ds.training.real_data_loaded", context={"dataset_hash": dataset_hash, "rows": len(df), "start": str(start_date), "end": str(end_date)})
    ...
    logger.log_event("finrl_pro_ds.training.features_assembled", context={"feature_set_id": asm.feature_set_id})
    ...
    if turnover_penalty > 0.0:
        ...
        logger.log_event("finrl_pro_ds.training.wrapper_applied", context={"wrapper": "TurnoverPenaltyWrapper", "coef": turnover_penalty})
    ...
    if module_versions.get("action.space") == "ALLOCATION_VECTOR_LONG_ONLY":
        ...
        logger.log_event("finrl_pro_ds.training.wrapper_applied", context={"wrapper": "SoftmaxAllocationWrapper"})
    ...
    if action_smoothing > 0.0:
        ...
        logger.log_event("finrl_pro_ds.training.wrapper_applied", context={"wrapper": "ActionSmoothingWrapper", "smooth_factor": action_smoothing})

    logger.log_event("finrl_pro_ds.training.environment_created", context={"env_name": "ProStockEnv"})
    ...
    if module_versions.get("action.space") == "ALLOCATION_VECTOR_LONG_ONLY":
        ...
        logger.log_event("finrl_pro_ds.training.agent_configured", context={"action_adapter": "identity"})
    ...
    logger.log_event("finrl_pro_ds.training.agent_instantiated", context={"agent_name": agent_name})
    ...
    for t in range(total_timesteps):
        ...
        if terminated or truncated:
            if not is_off_policy:
                ...
                logger.log_event("finrl_pro_ds.training.agent_updated", context=metrics_from_update)
...
def main(argv: Iterable[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="finrl_pro_ds.training.run_experiment",
    ...
    manifest = Path(cfg.get("fingerprint_manifest", "finrl_pro_ds/configs/fingerprints.yaml"))
    ...
    with mlflow.start_run(run_name=run_name) as active_run:
        ...
        if ds_hash.startswith("snapshot://") and not real_training_enabled:
            try:
                # from finrl_pro_ds.data.loader import DataLoader  # noqa: WPS433 (already imported)
                df = DataLoader.resolve_dataset(ds_hash)
                logger.log_event(
                    "finrl_pro_ds.training.dataset_resolved",
                    context={"dataset_hash": ds_hash, "rows": int(df.shape[0])},
                )
            except Exception as e:  # noqa: BLE001
                logger.log_event(
                    "finrl_pro_ds.training.dataset_resolve_error",
        ...
        if not real_training_enabled:
            ...
            if ds_hash.startswith("snapshot://") and feat_cfg:
                try:
                    # from finrl_pro_ds.data.loader_pro import ProFeatureAssembler  # noqa: WPS433 (already imported)
        ...
        if tcfg.get("register_model", False):
            model_name = str(tcfg.get("model_name", "finrl_pro_ds_model"))
