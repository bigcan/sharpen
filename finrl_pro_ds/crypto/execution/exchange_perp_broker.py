...
class ExchangePerpBroker:
    ...
    async def _execute_order(
        ...
        if self.order_type == "limit":
            ...
            # S527-cont Phase 2: snapshot pre-order position so we can detect
            # fills via signed position delta when fetch_order is unreliable
            # (Bybit demo returns Python None instead of an order dict, which
            # makes the fetch_order-based path think nothing filled and fires
            # a market fallback that double-trades).
    ...
    async def _detect_fill_after_cancel(
        ...
        exchange. Trusting the position delta closes the bug class for any
        venue whose order endpoint is unreliable. Falls back to
        ``fetch_order`` when the snapshot path is unusable so we don't
        regress against well-behaved venues.
            ...
            try:
                ...
                if total_equity > 0 and mid_price > 0:
                    ...
                    if delta_weight * expected_sign < 0:
                        # Position moved opposite to the order direction —
                        # could be a flatten by another path. Don't credit
                        # this delta to the cancelled limit; let the market
                        # fallback fire and Phase 1 reconcile catch over-fills.
                        logger.warning(
        ...
        # Fallback: fetch_order path (original behaviour, kept for venues
        # whose order endpoint is reliable or when the snapshot path fails).
        try:
