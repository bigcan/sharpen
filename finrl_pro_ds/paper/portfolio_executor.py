...
:class:`~finrl_pro_ds.paper.sleeves.AllocatorBookSleeve` (the ETF/IB account — momentum
⊕ rates, weight-combined internally, UNCHANGED) with zero or more
:class:`~finrl_pro_ds.paper.sleeves.ReturnStreamSleeve` (the Deribit VRP account) at the
...
Parity (mirrors :class:`~finrl_pro_ds.paper.two_sleeve.TwoSleeveExecutor`):
...
from finrl_pro_ds.paper.two_sleeve import _as_oracle
...
class PortfolioExecutor:
    ...
    def _build_sleeves(config: Mapping) -> list[Sleeve]:
        ...
        if vrp_cfg.get("enabled", False):
            from finrl_pro_ds.paper.sleeves import VRPSleeve
