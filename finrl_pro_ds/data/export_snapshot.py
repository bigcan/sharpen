...
def main(argv: Iterable[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="finrl_pro_ds.data.export_snapshot",
    ...
    logger = MLOpsLogger()
    logger.log_event("finrl_pro_ds.snapshot.export_start", context={"snapshot_id": args.id, "format": args.format})
    ...
    try:
        ...
        logger.log_event("finrl_pro_ds.snapshot.export_error", context={"error": str(e)})
        raise
    logger.log_event("finrl_pro_ds.snapshot.export_success", context={"path": str(path)})
