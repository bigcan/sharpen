...
class ModuleScaffolder:
    """Scaffold new modules within the finrl_pro_ds namespace."""
    ...
    def scaffold(self, dotted_path: str, *, docstring: str | None = None) -> Path:
        """Create a new module ensuring it resides within finrl_pro_ds."""
        if not dotted_path.startswith("finrl_pro_ds."):
            raise ValueError("Modules must be created under the finrl_pro_ds namespace.")
    ...
    def _iter_package_parents(path: Path) -> list[Path]:
        """Return all parents down to the finrl_pro_ds package."""
        packages: list[Path] = []
        current = path
        while current.name and current.name != "finrl_pro_ds":
