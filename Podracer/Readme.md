...
创建docker context 命令：
```
docker context create gpu_64 --docker host=tcp://<LAN_HOST>:2375
```
参数解释：
- `gpu_64` 是用户自己定义的 docker context 名称
- `tcp://<LAN_HOST>` 是作为计算节点使用的远程服务器IP地址
- `tcp://<LAN_HOST>:2375` 的2375 是远程服务器的端口号，用户可以自己随便指定一个
