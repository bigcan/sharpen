...
emitted by ``finrl_pro.training.commands.run_matrix`` and enforce the
...
from finrl_pro.mlops.risk_profiles import RiskControlProfile, load_risk_profile
...
def _resolve_config_path(config_path: str | Path) -> Path:
    ...
    experiments_root = Path("finrl_pro") / "configs" / "experiments"
    matches = list(experiments_root.rglob(f"{base}.yaml"))
    if matches:
        return matches[0].resolve()
    raise FileNotFoundError(
        f"Config '{config_path}' not found; expected under tmp inputs or finrl_pro/configs/experiments."
    )
...
def _load_experiment_metadata(config_path: str) -> ExperimentRiskMetadata:
    ...
    profile_file_raw = payload.get("risk_profile_file") or "finrl_pro/configs/risk_profiles.yaml"
