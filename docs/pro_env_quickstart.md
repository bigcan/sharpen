...
- Config flag (per experiment YAML):
  - File: `finrl_pro_ds/configs/experiments/<your_exp>.yaml`
  - Under `training:` set:
...
- Assemble arrays from a snapshot and create the Pro env:
  - `finrl_pro_ds/data/loader_pro.py` produces arrays and the `feature_set_id` by reusing the feature store keyed by `features.cache_key`.
  - `finrl_pro_ds/envs/factory.py` wraps the dynamic env creation.
...
- Reproducibility:
  - `features.cache_key` + `features.feature_set_id` are logged in `module_versions` (see `finrl_pro_ds/training/run_experiment.py`).
