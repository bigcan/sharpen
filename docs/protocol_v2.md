...
- **Mechanism (ADR-1/2 — data layer, NOT an env wrapper; corrects the forward-declared sketch):** for each masked test-region bar and channel `c∈{O,H,L,C}`: `x'_c = x_c · exp(ε_c)`, `ε_c ~ N(0,σ²)` i.i.d.; min/max ordering repair `H' = max(O',H',L',C')`, `L' = min(O',H',L',C')`; volume passthrough (ADR-8). The noised 1-min bars are materialized to a parquet and the cell config's `data.file_path` is pointed at it; the **unmodified** `MultiScaleOHLCVHandler` recomputes every feature, the X2 causal coarse-bar map, EMA-Z and ATR from them (single source of truth — `finrl_pro_ds/eval/obs_noise.py::apply_ohlc_noise`). **Zero diff to `multiscale_handler.py` / `continuous_swing_env.py` / `env_factory.py`** (chosen partly because the handler is under fleet-halt leak re-validation).
