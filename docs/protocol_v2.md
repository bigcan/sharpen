...
>
> **Version history:**
> - **v2.7-B** (2026-06-02, S553-cont-25) — **Stage 3.5 observation-noise robustness SHIPPED (Phase α).** New eval-only stage between Stage 3 WF and Stage 4 OOS (Researcher Method #4 GO): re-rolls the frozen PROMOTE ensemble through **price-path-randomized** OHLC — per-channel multiplicative log-noise `x' = x·exp(N(0,σ²))` on the raw 1-min bars, min/max ordering-repaired, volume passthrough — then recomputes every feature / the X2 causal map / EMA-Z / ATR via the **unmodified** handler from a materialized noised parquet (single source of truth — **ADR-1/2, NOT an env wrapper**, correcting the forward-declared sketch). Per (fold, σ): `pf_ratio = median_k(PF) / PF_nominal`, `mdd_degradation_pp = q95(|MDD_seeds|) − |MDD_nominal|` (**abs BEFORE the quantile — MATH-OBS-4**; the gate-defeating `|q95(signed)|` reports the rosy tail and is a CRITICAL bug). Worst-fold gate: `min_f pf_ratio ≥ obs_noise_pf_floor_σ` AND `max_f mdd_degradation_pp ≤ obs_noise_mdd_buffer_pp_σ` ∀σ. New manifest `obs_noise_report.json` (schema "1.0") on a **dedicated** Stage 3.5 WandB run under `obs_noise/*` (ADR-10). New validator rule `check_obs_noise_gate` (registered at the `wf` stage; dormant for `protocol_version < 2.7` via the N4 version-tuple compare). σ-levels `{10bps: 0.001, 50bps: 0.005}`, `n_seeds: 10`, floors `0.85/0.70`, buffers `0.3/0.7 pp` added to 6 ensemble gates yamls with `obs_noise_required: false`. **Phase α (S553-cont-25+) — keys present, validator dormant, ZERO live-strategy impact**; Phase β — operator locks the floors/buffers on the de-leaked candidates' empirical spread (**gmgp1-btc X2-de-leak = first canary**), then bumps `protocol_version: "2.7"` + `obs_noise_required: true`. **No auto-unship on FAIL** (a FAIL = path-fragile edge → queue retrain / tighten live drift gates). New invariants OBSNOISE-1..4 + Math-promoted MATH-OBS-1..5 (per-`(fold,σ,k)` seed via `np.random.SeedSequence`, never `hash()` — MATH-OBS-5). Standalone CLI `scripts/stage_3_5_obs_noise.py` (canonical; no `run_full_pipeline --stage` dispatcher). Rollouts run **serially** in this cut (rule_fn closures aren't ProcessPool-picklable; CUDA-in-subprocess sharing risks the documented multiprocessing race). **Architecture** `.agent/artifacts/protocol_v27_b_obs_noise_stage_3_5_architecture.md` (10 ADRs, OBSNOISE-1..4, **Math gate PASS WITH NOTES**, 5-commit B1–B5). v2.7-C (Stage 3 exec-failure stress, Method #3) remains forward-declared.
> - **v2.7-A N2** (2026-05-31) — **PF-XCHECK dual-equity closure + gates-driven enforcement.** The env records an independent `(H+L)/2`-marked shadow-equity curve in lockstep with the close-marked curve behind `record_dual_equity` (set by `run_cell`); per-cell PF-XCHECK now computes a real `|pf_close − pf_mid| / pf_close` divergence (closes the deferred `SKIPPED` path; N2 impl `3732c38d`, S553-cont-11). The report-only mode and divergence threshold — previously module constants `PF_XCHECK_REPORT_ONLY` / `PF_XCHECK_DIVERGENCE_HALT` — are promoted to gates keys `gates.pf_xcheck_report_only` (default **true**, Phase α) and `gates.pf_xcheck_divergence_halt` (default **0.30**), resolved by `_resolve_pf_xcheck_gates` and threaded through `run_cell` (constants demoted to defaults; honors CLAUDE.md "no hardcoded gate thresholds"). Verdict block records `pf_xcheck_config` provenance; `check_sensitivity_audit` validates both keys and FAILs prop-firm Phase-β configs (`sensitivity_audit_required: true`) omitting an explicit `pf_xcheck_report_only`. Keys added to all 9 ensemble gates yamls (additive, behavior-preserving — defaults equal the prior constants). **Calibration re-anchored to the 2026-05-31 data-leak retrain:** the report-only → enforce flip is locked per workstream against each *retrained (de-leaked)* policy's observed divergence at its Stage 2.5-R run, not the pre-retrain deployed set. Design: architecture doc N2 addendum §N2.1–N2.12 (Math-verified MATH-N2-b — report-only calibration is mathematically required before enforce).
...
#### Grid
- 3×3 over `(deadband_threshold, max_leverage)` YAML knobs (the two config knobs that change the env's action-processing layer at [`continuous_swing_env.py:245,286,298-303`](../sharpen/envs/continuous_swing_env.py#L243-L313) without retraining).
...
- **PF-XCHECK per-cell** (CLAUDE.md invariant): close-marked PF vs an independent `(H+L)/2`-marked shadow-equity PF. **N2 landed** (`3732c38d`, S553-cont-11): the env records the shadow curve in lockstep behind `record_dual_equity` (set by `run_cell`), so PF-XCHECK computes a real `|pf_close − pf_mid| / pf_close` divergence instead of `SKIPPED`. The mode + threshold are **gates-driven** (v2.7-A): `gates.pf_xcheck_report_only: true` (Phase α) surfaces the divergence but never halts; flipping `false` makes `> gates.pf_xcheck_divergence_halt` (default `0.30`) flag `halt: true` and drop the cell from the edge-stability gate. Both default to the prior module constants when a key is absent (back-compat). SKIPPED still applies only when no `pv_mid` is recorded (flag off in an upstream path). Design: architecture doc N2 addendum + §N2.12.
...
|---|---|---|---|
| **α — calibration** | `protocol_version` defaults to `"2.5"`; gates yamls declare keys with `sensitivity_audit_required: false` AND `pf_xcheck_report_only: true` (PF-XCHECK divergence surfaced, never halts). | Dormant (back-compat exempt). | Per **retrained (de-leaked)** policy at its Stage 2.5-R run — re-anchored from the pre-retrain 5-deployed set by the 2026-05-31 data-leak retrain. Operator reviews (a) the empirical `pf_ratio` distribution to lock `edge_stability_pf_ratio_floor`, and (b) the per-cell PF-XCHECK divergence distribution to confirm `pf_xcheck_divergence_halt` (0.30) clears with margin. |
| **β — enforcement** | Operator bumps L1 multiseed `protocol_version: "2.6"`, flips `gates.sensitivity_audit_required: true`, AND flips `gates.pf_xcheck_report_only: false` once the divergence threshold is locked. | Validator FAILs prop-firm configs missing the gates keys (incl. an explicit `pf_xcheck_report_only` when required); Stage 3 launcher refuses without `sensitivity_audit` block in upstream manifest. | Permanent. |

#### Operator backfill flow (Phase α calibration)
> **2026-05-31 re-anchor (data-leak retrain):** the example loop below predates the data-leak retrain. Calibration now runs **per retrained policy** as each de-leaked Stage 2.5-R completes; the workstream set + seeds are whatever the retrain produces, not the frozen list shown. Both `edge_stability_pf_ratio_floor` and `pf_xcheck_divergence_halt` are locked from the *retrained* policy's distributions before flipping the respective enforce flags.
```
...
### Stage 3.5 — observation-noise robustness (v2.7-B SHIPPED, Phase α)
**Eval-only stage between Stage 3 WF and Stage 4 OOS (Researcher Method #4 GO).** Re-rolls the frozen PROMOTE ensemble through **price-path-randomized** OHLC and gates on edge survival under perturbation — the ex-ante structural answer to the sim-to-live tail-DD class of incidents ([project_sim_to_live_gap_audit_s470], [project_live_obs_normalization_mismatch_s509]).

- **Mechanism (ADR-1/2 — data layer, NOT an env wrapper; corrects the forward-declared sketch):** for each masked test-region bar and channel `c∈{O,H,L,C}`: `x'_c = x_c · exp(ε_c)`, `ε_c ~ N(0,σ²)` i.i.d.; min/max ordering repair `H' = max(O',H',L',C')`, `L' = min(O',H',L',C')`; volume passthrough (ADR-8). The noised 1-min bars are materialized to a parquet and the cell config's `data.file_path` is pointed at it; the **unmodified** `MultiScaleOHLCVHandler` recomputes every feature, the X2 causal coarse-bar map, EMA-Z and ATR from them (single source of truth — `sharpen/eval/obs_noise.py::apply_ohlc_noise`). **Zero diff to `multiscale_handler.py` / `continuous_swing_env.py` / `env_factory.py`** (chosen partly because the handler is under fleet-halt leak re-validation).
- **Noise region (ADR-5 / LEAK-1):** `noise_start = test_start` (asserted `≥ val_end`); train+val EMA-Z warmup bars stay byte-identical to source (OBSNOISE-2). σ is log-space; `bps = σ·1e4` (MATH-OBS-2). Per-`(fold, σ-level, k)` seeds via `np.random.SeedSequence`, never `hash()` (MATH-OBS-5).
- **Aggregation + gate (ADR-6):** per fold a `σ=0` nominal rollout (in-band reference via the identical code path, ADR-3) + per σ-level × `gates.obs_noise_n_seeds` (default **10**) noisy rollouts. `pf_ratio = median_k(PF)/PF_nominal`; `mdd_degradation_pp = q95(|MDD_seeds|) − |MDD_nominal|` (**abs BEFORE the quantile — MATH-OBS-4**). PASS iff `min_f pf_ratio ≥ gates.obs_noise_pf_floor_<σ>` (default **0.85**/10bps, **0.70**/50bps) AND `max_f mdd_degradation_pp ≤ gates.obs_noise_mdd_buffer_pp_<σ>` (default **0.3**/**0.7** pp) ∀σ. Folds with `PF_nominal < 1.0` excluded (anomaly, `PF_nominal == PF_CAP` flagged); graded folds below `gates.obs_noise_required_min_folds` → `UNKNOWN_INSUFFICIENT_FOLDS`.
- **Artifact + run:** `obs_noise_report.json` (schema "1.0") on a **dedicated** Stage 3.5 WandB run under `obs_noise/*` (ADR-10). Consumed by Stage 4's upstream-manifest check for prop-firm.
- **Rollout:** standalone CLI `scripts/stage_3_5_obs_noise.py` (canonical; ADR-7 — no `run_full_pipeline --stage`). **Serial** in this cut (rule_fn closures non-picklable across a ProcessPool; CUDA-subprocess sharing risks the documented multiprocessing race); scratch is bounded (generate → roll → delete per cell unless `--keep-noised`). ~`K_folds × (1 + Σ_σ·n_seeds)` ensemble rollouts/workstream.
- **Rollout phasing (ADR-9):** Phase α (S553-cont-25+) — `obs_noise_required: false` in 6 ensemble gates yamls, validator `check_obs_noise_gate` dormant for `protocol_version < 2.7` (N4 compare), **zero live impact**. Phase β — operator locks floors/buffers on the de-leaked candidates' empirical spread (**gmgp1-btc X2-de-leak = first canary**), then bumps `protocol_version: "2.7"` + `obs_noise_required: true`. **No auto-unship on FAIL** — a FAIL = path-fragile edge → queue retrain / tighten live drift gates.
- **Invariants:** OBSNOISE-1 (ordering repair), OBSNOISE-2 (pre-region byte-identical), OBSNOISE-3 (σ=0 identity → nominal reproduces Stage 3), OBSNOISE-4 (deterministic, causal positional draw — bar `t`'s noise depends only on `(seed, row index)`, never on bars `>t`); Math-promoted MATH-OBS-1..5. Architecture + Math gate (PASS WITH NOTES): `.agent/artifacts/protocol_v27_b_obs_noise_stage_3_5_architecture.md`. Researcher: `.agent/artifacts/mc_robustness_methods_research.md` §Method #4.
...
- **Prop-firm / live-capital (v2.4):** deploy YAML = base training config + `configs/deploy/<firm>/<phase>.yaml` overlay layered via `sharpen/config_utils.deep_merge(allowlist=...)`. Base must use `env.risk:` (not deprecated `env.prop_firm:`). Overlay supplies the `challenge:` block (`challenge.enabled=true`, `challenge.phase ∈ {step1, step2, funded, custom}`, `challenge.advance_rule: "manual_ack"`). `challenge.advance_rule: "auto"` FAILs validator. Overlay keys restricted by `configs/deploy/ALLOWLIST.yaml` — touching `env.reward.*` or any HPO-sensitive key FAILs before merge.
