...
*BOTH corrected-contract MDE curves were measured end-to-end on 2026-07-30
(`calibration_mde_sweep_corrected.json` 8 cells, `calibration_xsec_mde_sweep_corrected.json` 24 cells), so
this section can be stated as numbers. Running the PRODUCTION stamp (worst MDE across the types a tick
mines) against the 0.50 ceiling: cross_asset holdout 1163 → **1.683** · taiwan holdout 1011 → **1.833** ·
...
(`periods_per_year=252`), so applying a high-`T` row to an intraday substrate assumes comparable per-bar
SNR, which the sweep does not measure. Validating that is now the highest-value U8 experiment. Readiness
write-up: `docs/research/crucible_rediscovery_readiness_2026-07-30.md`.*
...
| NULL-DEGEN-01 | ~~MEDIUM-HIGH~~ **LOW (measured 2026-07-31)** | `scripts/research/crucible_calibration.py:137,215`; `scripts/research/crucible_orchestrator.py:164` | **NEW 2026-07-31, and it reaches further than RC-11 did.** All three null/synthetic panel generators build the timing slot as `sin(2π·t/80) + 0.2·noise` — fixed period, fixed phase, identical on every seed. Any statistic computed over OVERLAY null draws on those substrates has an effective n near the *formula* count rather than the draw count. **`run_e1` (line 467/472) scores `_OVERLAY_NULL_SEEDS` on exactly these panels**, so E1's per-candidate null FPR inherits it: its Clopper-Pearson bound assumes independent Bernoulli trials that the overlay half does not supply, and the recorded result sits ~2% from its ceiling (per-tick 0.025, CP-upper95 0.0487 vs 0.05 — "one more false positive would have flipped it"). **E1 WAS re-run both ways (240 panels, corrected, realistic null) and is UNAFFECTED**: identical per-tick FPR 0.0208 / CP95 0.0433 with fixed vs randomized slots under `offspring_policy: all` (the regime that produces any false positives at all), and 0.0000 / CP95 0.0124 under the shipped `prereg_only`. Mechanism: degeneracy needs BOTH the signal and its target fixed, and E1 rebuilds proxy base sleeves from each panel's own prices, so the tilt's target varies and the draws stay independent through the base book. The U7 uplift null pointed the same sinusoid at ONE fixed real substrate — both halves frozen — hence its 11.8 variance ratio. **Downgraded to LOW and re-scoped**: a latent trap for any future "measure the null on the real base book" harness, not a live defect. Opt-in defences shipped (`--randomize-slots` on E1, `--randomize-regime` on the uplift null) plus the between/within diagnostic as detector. The *planted* panel (`:265`) shares the sinusoid but there it is the signal by design — that is a generalization caveat on the MDE curves ("MDE for a slow-sinusoidal timing signal"), not a degeneracy. |
