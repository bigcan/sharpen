...
**Status:** **BUILT AND SHIPPED — system is at `crucible-v13.0`** (P0–P5 roadmap complete). §§0–11 are the original design spec (written 2026-07-01, last revised 2026-07-14 at `crucible-v2.6`) and remain accurate for the **loop shape**; the **decision layer** has moved eleven times since and is assembled in **§12**, which supersedes any verdict-semantics statement above it. **Revised per independent Fable-5 review** (`.agent/artifacts/crucible_spec_fable_review.md`, 2026-07-01) — two critical design fixes folded in; see §11.
...
## 12. Decision contract as of `crucible-v13.0` (assembled 2026-08-11)

§§0–11 above were frozen 2026-07-14, when the system was at `crucible-v2.6`, and they describe the **loop shape** — which is still accurate: no bump since has changed the stage graph. What they do not describe is the **decision layer**, which has changed eleven times since (v3.0 → v13.0). Until now that record lived only in the `finrl_pro_ds/crucible/version.py` module docstring plus the audit reports, so no single artifact showed a screen and the gate it feeds side by side. That is not a documentation nicety: the same defect — *a cheap upstream screen silently blocking the gate that is the actual test* — has now shipped four separate times (§12.5), and each instance was found by measurement long after the fact.
