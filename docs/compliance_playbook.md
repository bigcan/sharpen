...
2. Ensure MLflow credentials and the DVC remote are configured:
   ```bash
   export $(grep -v '^#' conf/finrl_pro_ds.env | xargs -0)
   dvc remote list
   ```
...
- Define profiles in `finrl_pro_ds/configs/risk_profiles.yaml` with exposure,
  turnover/cost, sandbox, and fallback parameters.
- Log profiles to the control plane using the API client:
  ```python
  from finrl_pro_ds.mlops.api_client import FinRLProAPIClient
...
1. Instantiate the trainer with the fingerprint store tied to the current
   ...
   profile = RiskControlProfile(
       ...
       fallback_agent="finrl_pro_ds.agents.baseline",
...
2. Run training with deterministic seeds and record artifact URIs:
   ...
   fingerprint = trainer.run(
       ...
       seed=20250101,
       module_versions={"finrl_pro_ds.training.trainer": "commit-sha"},
       metrics={
           "sharpe_ratio": 1.12,
           "max_drawdown": 0.12,
           "volatility": 0.26,
...
- Replay the fingerprint using the CLI and archive the JSON output:
  ...
  python -m finrl_pro_ds.training.commands.reproduce $FINGERPRINT_ID \
    --manifest finrl_pro_ds/configs/fingerprints.yaml > artifacts/fingerprint.json
  ```
