...
load-bearing parity check. Two kinds:
  ...
  - :class:`AllocatorBookSleeve` — weights over the ETF union (an IB account): the
    existing momentum+rates :class:`~sharpen.paper.two_sleeve.TwoSleeveExecutor`,
...
:class:`~sharpen.paper.portfolio_executor.PortfolioExecutor` combines these at the
...
from sharpen.crypto.options_vrp_sim import SimConfig, returns_from_pnl, simulate_asset
...
class AllocatorBookSleeve(Sleeve):
    """Adapter wrapping :class:`~sharpen.paper.two_sleeve.TwoSleeveExecutor` as one
