...
linear-core** sleeve return streams a generated candidate must actually improve:
  ...
  * ``tsmom``        — the cross-asset time-series-momentum sleeve (multi-look-back
    ...
    :func:`sharpen.features.cross_asset_signals.compute`; defaults LOCKED to the
  ...
  * ``rates_carry``  — the Treasury-curve carry+roll sleeve over {SHY,IEF,TLT,LQD} from
    :func:`sharpen.features.rates_carry.rates_carry_conviction`, vol-scaled with the
...
NOT route through the env-rendered :class:`~sharpen.paper.two_sleeve.TwoSleeveExecutor`
...
def production_base_sleeves(
    ...
    *,
    ...
    """
    from sharpen.data.treasury_curve_loader import load_treasury_curve_with_manifest
...
def _load_taiwan_futures_close(
    ...
    *,
    ...
    """
    if fetch_fn is None:
        from sharpen.data.taiwan_panel_loader import fetch_taiwan_wide  # noqa: PLC0415
...
def taiwan_tsmom_sleeve_returns(
    ...
    *,
    ...
    """
    if futures is None:
        from sharpen.data.taiwan_panel_loader import load_taiwan_universe  # noqa: PLC0415
