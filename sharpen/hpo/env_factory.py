...
def _build_env(config, start_date=None, end_date=None, shm_config=None, norm_cutoff_date=None):
    ...
    if mdp_version in ("v8", "v9"):
        ...
        if handler_type == "lob_micro":
            from sharpen.data.lob_micro_handler import LOBMicroDataHandler
            ...
            from sharpen.data.lob_data_handler import LOBDataHandler
            ...
            from sharpen.data.mm_data_handler import MMDataHandler
    ...
    if mdp_version == "cmgp1":
        ...
        if not hasattr(make_env, cache_key):
            from sharpen.crypto.data.crypto_loader import fetch_crypto_data
