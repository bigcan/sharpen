...
- Reproducibility assets: T006 and T017 establish fingerprint storage in `finrl_pro_ds/configs/fingerprint_store.py`.
- Risk controls: T007 and T024 define and enforce risk profiles via `finrl_pro_ds/mlops/risk_controls.py` and `finrl_pro_ds/training/trainer.py`.
- Evaluation harness: T008 and T033 implement walk-forward evaluators in `finrl_pro_ds/eval/base.py` and `finrl_pro_ds/eval/walk_forward.py`.
- Observability: T009 and T026 expand structured logging and alerting in `finrl_pro_ds/mlops/logger.py` and `finrl_pro_ds/mlops/alerting.py`.
...
- [X] T003 [P] Create MLflow and DVC environment sample configuration in `conf/finrl_pro_ds.env.example`
...
- [X] T006 Implement fingerprint manifest store skeleton in `finrl_pro_ds/configs/fingerprint_store.py`
- [X] T007 [P] Scaffold risk control base contracts in `finrl_pro_ds/mlops/risk_controls.py`
- [X] T008 [P] Define evaluation harness base interface in `finrl_pro_ds/eval/base.py`
- [X] T009 Expand structured observability baseline in `finrl_pro_ds/mlops/logger.py`
...
**Independent Test**: Run the scaffolder to generate a new agent under `finrl_pro_ds/agents/` and confirm the diff touches only FinRL Pro directories while registering the module metadata.

- [X] T010 [US1] Implement `FinRLProModule` dataclass and registry operations in `finrl_pro_ds/utils/module_registry.py`
- [X] T011 [US1] Seed module manifest with change-control metadata in `finrl_pro_ds/configs/modules.yaml`
- [X] T012 [US1] Build module scaffolding utility for FinRL Pro namespaces in `finrl_pro_ds/utils/module_scaffolder.py`
- [X] T013 [US1] Wire CLI entrypoint for module scaffolding in `finrl_pro_ds/__main__.py`
...
**Independent Test**: Execute `finrl_pro_ds.training.reproduce` with a stored fingerprint and verify regenerated metrics fall within the ±2% baseline variance.

- [X] T016 [US2] Implement `ExperimentFingerprint` dataclass with validation in `finrl_pro_ds/mlops/fingerprint.py`
- [X] T017 [US2] Persist fingerprints and config hashes via manifest updates in `finrl_pro_ds/configs/fingerprint_store.py`
- [X] T018 [US2] Instrument trainer to log MLflow runs, DVC artifacts, and fingerprint metadata in `finrl_pro_ds/training/trainer.py`
- [X] T019 [US2] Add reproducibility integration test covering fingerprint replay in `tests/integration/test_reproducibility.py`
- [X] T020 [US2] Create reproducibility CLI command for pipeline replays in `finrl_pro_ds/training/commands/reproduce.py`
- [X] T021 [US2] Implement `/experiments` API client wrappers in `finrl_pro_ds/mlops/api_client.py`
- [X] T022 [US2] Implement `/evaluations` API client wrappers in `finrl_pro_ds/mlops/api_client.py`
...
- [X] T024 [US3] Implement `RiskControlProfile` dataclass and validation in `finrl_pro_ds/mlops/risk_profiles.py`
- [X] T025 [US3] Enforce risk thresholds and sandbox toggles during runs in `finrl_pro_ds/training/trainer.py`
- [X] T026 [US3] Add drawdown and anomaly logging hooks in `finrl_pro_ds/mlops/logger.py`
- [X] T027 [US3] Create alert routing utilities for risk events in `finrl_pro_ds/mlops/alerting.py`
- [X] T028 [US3] Add integration test simulating risk breaches in `tests/integration/test_risk_controls.py`
- [X] T029 [US3] Extend API client with `/risk-profiles` operations in `finrl_pro_ds/mlops/api_client.py`
...
- [X] T030 [US4] Implement `BenchmarkCatalogEntry` dataclass and validation in `finrl_pro_ds/eval/benchmark_catalog.py`
- [X] T031 [US4] Persist benchmark catalog store helpers in `finrl_pro_ds/eval/benchmark_catalog.py`
- [X] T032 [US4] Seed baseline benchmark manifest in `finrl_pro_ds/configs/benchmarks.yaml`
- [X] T033 [US4] Complete walk-forward evaluation pipeline with benchmark catalog integration in `finrl_pro_ds/eval/walk_forward.py`
- [X] T034 [US4] Integrate SHAP explainability outputs into reports in `finrl_pro_ds/explainability/shap_analysis.py`
- [X] T035 [US4] Build compliance report orchestration pipeline in `finrl_pro_ds/eval/report_pipeline.py`
- [X] T036 [US4] Add evaluation reporting integration test in `tests/integration/test_reporting.py`
- [X] T037 [US4] Implement `/reports` API client coverage in `finrl_pro_ds/mlops/api_client.py`
...
- After T002, run T003 and T007 in parallel—they touch distinct files (`conf/finrl_pro_ds.env.example`, `finrl_pro_ds/mlops/risk_controls.py`).
...
- In US3, T027 and T026 can run in parallel once T024 lands because they target separate files (`finrl_pro_ds/mlops/alerting.py`, `finrl_pro_ds/mlops/logger.py`).
