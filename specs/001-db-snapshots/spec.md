...
**Created**: 2025-11-06  
**Status**: Draft  
**Input**: User description: "Implement a database-backed data snapshot system for FinRL Pro: store OHLCV bars in TimescaleDB (market_bars hypertable), add snapshots + snapshot_assets tables, CLI finrl_pro_ds.data.snapshot (fetch via yfinance/Alpaca, upsert to DB, record metadata: snapshot_id, params_json, lib_versions, code_hash), CLI finrl_pro_ds.data.export_snapshot (DB→Parquet/CSV), resolve dataset_hash 'snapshot://<snapshot_id>' in finrl_pro_ds.data.loader, integrate with Trainer + README, and add tests"
...
**Independent Test**: Run `python -m finrl_pro_ds.data.snapshot --provider yahoo --tickers SPY,AAPL --start 2020-01-01 --end 2024-01-01 --interval 1d` and verify: (a) rows are present in `market_bars`; (b) a new entry in `snapshots` with correct params and `snapshot_id` is created; (c) CLI prints the `snapshot_id` JSON.
...
**Independent Test**: Run `python -m finrl_pro_ds.data.export_snapshot --id <snapshot_id> --out data/snapshots --format parquet` and verify the file exists, row count matches DB, and checksum is recorded.
...
- **FR-002 (Snapshot CLI)**: Implement `finrl_pro_ds.data.snapshot` CLI to fetch (yfinance/Alpaca), upsert bars, insert a `snapshots` row, and print JSON `{snapshot_id, row_count}`.
- **FR-003 (Export CLI)**: Implement `finrl_pro_ds.data.export_snapshot` CLI to materialize a snapshot to Parquet/CSV with checksum and output path.
- **FR-004 (Loader Resolution)**: Enhance `finrl_pro_ds.data.loader` to resolve `dataset_hash` values of the form `snapshot://<snapshot_id>` by querying the database and returning a standardized DataFrame.
...
- **FR-007 (Secrets/Config)**: Read provider credentials from `conf/finrl_pro_ds.env`; do not commit secrets; fail with clear messages if missing.
...
- Extension boundary respected? Yes — all new code under `finrl_pro_ds/**`; no changes to `FinRLPodracer/**`.
