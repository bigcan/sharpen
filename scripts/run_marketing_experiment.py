...
def train_agent(df, features, run_name, seed=42, verbose=True):
    ...
    try:
        agent = PPO("MlpPolicy", e_train_gym, verbose=0, seed=seed, policy_kwargs=dict(net_arch=[64, 64]))
...
def main():
    ...
    # ---------------------------------------------------------
    # Experiment B: Hybrid Feature Factory (mRMR)
    # ---------------------------------------------------------
    print("\n=== EXPERIMENT B: Hybrid Feature Factory ===")
    ...
    time_b, val_b, agent_b, env_b = train_agent(df_b, final_features, "Hybrid Factory")
    
    # ---------------------------------------------------------
    # Experiment C: Ensemble (3 Seeds)
    # ---------------------------------------------------------
    print("\n=== EXPERIMENT C: Ensemble (3 Seeds) ===")
    ...
    for seed in seeds:
        ...
        dt, val, ag, _ = train_agent(df_b, final_features, "Ensemble Member", seed=seed, verbose=False)
        if ag:
