#!/usr/bin/env python
"""GMGP1-XAUUSD Extended-Window — Stage 3 WF dispatcher (multiseed N=3).

Clone of `scripts/launch_sg1_eurusd_hg_wf_multiseed.py` (S549-cont pattern),
retargeted to GMGP1-XAUUSD post Tier B2 L1 N=10 CLEAN PASS (S550-cont-8).
For each fold (8 emitted by splitter on 2024-01-01 → 2026-04-15 envelope at
18mo train / 1mo val / 1mo test / 1mo step):
  ...
  3. Launch via `scripts/launch_l1_multiseed.py` with --seeds 789,2025,1024.
Cells run sequentially; within a cell, seeds fan out across slots.

After this dispatcher completes (24 train runs done), run:
  python scripts/gmgp1_xauusd_ensemble_eval_oanda.py \
...
Usage:
    ...
    python scripts/launch_gmgp1_xauusd_wf_extended.py \
        --config configs/gmgp1_xauusd_wf_extended.yaml \
        --dry_run

    # Full launch across gpuhub-1+2 4-slot pool (1 GPU per slot, no contention)
    python scripts/launch_gmgp1_xauusd_wf_extended.py \
        --config configs/gmgp1_xauusd_wf_extended.yaml \
        --slots gpuhub-1:0,gpuhub-1:1,gpuhub-2:0,gpuhub-2:1
...
Manifest: writes `results/gmgp1_xauusd_wf_extended_<timestamp>/manifest.json`
with per-fold (study_id, seeds, exit_code, wandb_finished_ok) entries for the
post-run analyzer. WandB tags per cell:
  `gmgp1-xauusd`, `XAUUSD`, `wf-stage3`, `FTMO`, `extended-window`,
  `fold-<i>`, `study-id-<ts>`.
"""
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] gmgp1_xauusd_wf_extended - %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
log = logging.getLogger("gmgp1_xauusd_wf_extended")
...
def write_fold_config(base: dict, fold_idx: int, fold: dict, study_id: str) -> Path:
    ...
    out = RUNTIME_DIR / f"gmgp1_xauusd_wf_extended_fold_{fold_idx}.yaml"
...
def launch_cell(cfg_path: Path, fold_idx: int, slots: str | None,
    ...
    prefix = f"gmgp1-xauusd-ext-wf-fold{fold_idx}"
...
def main() -> int:
    ...
    results_dir = PROJECT_ROOT / "results" / f"gmgp1_xauusd_wf_extended_{timestamp}"
    results_dir.mkdir(parents=True, exist_ok=True)

    log.info("=" * 70)
    log.info("GMGP1-XAUUSD Extended Stage 3 WF Dispatcher (multiseed N=3)")
    ...
    log.info("Next: python scripts/gmgp1_xauusd_ensemble_eval_oanda.py "
             "--wf_config %s --gates_file configs/gmgp1_xauusd_extended_ensemble.gates.yaml",
