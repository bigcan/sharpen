...
def generate_report(run_id, output_path):
    ...
    metrics = {
        ...
        "Url": data.get("url"),
        "ValSharpe": round(summary.get("backtest_validation/sharpe", summary.get("backtest/validation_sharpe", 0.0)), 2),
        "TestSharpe": round(summary.get("backtest_test/sharpe", summary.get("backtest/test_sharpe", 0.0)), 2),
        "ValReturn": round(summary.get("backtest_validation/total_return", 0.0) * 100, 2),
        "TestReturn": round(summary.get("backtest_test/total_return", 0.0) * 100, 2),
        "ValMaxDD": round(summary.get("backtest_validation/max_drawdown", 0.0) * 100, 2),
