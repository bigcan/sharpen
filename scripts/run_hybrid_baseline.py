...
def compute_sharpe_from_returns(returns_csv_path: Path) -> float:
    if not returns_csv_path.exists(): return -999.0
    returns = []
...
def main():
    ...
    # Configure for "Hybrid Baseline" Mode
    ...
    # Enable Hybrid Baseline
    base_config["features"] = {
        "hybrid_baseline": True,
        "families": {},
        "advanced": {},
    ...
    seeds = [41, 42, 43]
    print("Starting Hybrid Baseline Validation (MACD/RSI/VWAP/ATR/Vol)")
    print("==========================================================")

    for seed in seeds:
        print(f"\nRunning Seed {seed}...")
        base_config["training"]["seed"] = seed
        base_config["experiment_id"] = f"phase2_hybrid_baseline_seed_{seed}"
        
        temp_config_path = Path(f"finrl_pro/configs/experiments/phase2_revamp/hybrid_baseline_seed_{seed}.yaml")
