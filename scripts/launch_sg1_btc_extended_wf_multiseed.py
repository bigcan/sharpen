#!/usr/bin/env python
"""SG-1-BTC Velotrade Stage 3 WF dispatcher — EXTENDED window (FU-3, multiseed N=3).

Mirrors `scripts/launch_btc_along_wf_multiseed.py` cell-based pattern. For each
fold (8 emitted by splitter on 2024-01-01 → 2026-04-06 envelope at 18mo train /
1mo val / 1mo test / 1mo step):
  ...
  3. Launch via `scripts/launch_l1_multiseed.py` with --seeds 456,3141,123
...
Usage:
    ...
    python scripts/launch_sg1_btc_extended_wf_multiseed.py \
        --config configs/sg1_btc_velotrade_rehpo_wf_multiseed_extended.yaml --dry_run

    # Full launch across both gpuhub-1 GPUs (3-tenant per fold cell, 4 slots free)
    python scripts/launch_sg1_btc_extended_wf_multiseed.py \
        --config configs/sg1_btc_velotrade_rehpo_wf_multiseed_extended.yaml \
        --slots gpuhub-1:0,gpuhub-1:0,gpuhub-1:1,gpuhub-1:1

    # Subset (e.g., only fold 0 for smoke)
    python scripts/launch_sg1_btc_extended_wf_multiseed.py \
        --config configs/sg1_btc_velotrade_rehpo_wf_multiseed_extended.yaml \
        --folds 0 --slots gpuhub-1:0

Manifest: writes `results/sg1_btc_extended_wf_<timestamp>/manifest.json` with
per-fold (study_id, seeds, exit_code, wandb_finished_ok) entries for the
post-run analyzer. WandB tags per cell:
  `sg-1`, `BTC`, `wf-stage3`, `extended-window`, `s535-cont-2`, `fu-3`,
  `fold-<i>`, `study-id-<ts>`.
"""
...
RUNTIME_DIR = PROJECT_ROOT / "configs" / "_sg1_btc_extended_wf_runtime"
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] sg1_btc_extended_wf - %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
log = logging.getLogger("sg1_btc_extended_wf")
...
def write_fold_config(base: dict, fold_idx: int, fold: dict, study_id: str) -> Path:
    ...
    out = RUNTIME_DIR / f"sg1_btc_extended_wf_fold_{fold_idx}.yaml"
...
def launch_cell(cfg_path: Path, fold_idx: int, slots: str | None,
    ...
    prefix = f"sg1-btc-extended-wf-fold{fold_idx}"
...
def main() -> int:
    ...
    p.add_argument("--slots", default=None,
                   help="Cross-GPU slot pool: comma-separated host:gpu pairs. "
                        "Recommended for 8-fold WF: 4 slots (2 per gpuhub-1 GPU).")
    ...
    results_dir = PROJECT_ROOT / "results" / f"sg1_btc_extended_wf_{timestamp}"
    results_dir.mkdir(parents=True, exist_ok=True)

    log.info("=" * 70)
    log.info("SG-1-BTC Stage 3 WF Dispatcher — EXTENDED window (FU-3, multiseed N=3)")
    ...
    # training finishes. Without WandB polling between cells, all 8 folds would
