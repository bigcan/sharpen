#!/usr/bin/env python
"""SG-1-XAUUSD Volume Study v2 Phase 2 — Stage 3 WF dispatcher (multiseed N=3).

Direct clone of `scripts/launch_sg1_btc_extended_wf_multiseed.py` (S535-cont-2,
FU-3 pattern). For each fold (8 emitted by splitter on 2024-01-01 → 2026-04-15
envelope at 18mo train / 1mo val / 1mo test / 1mo step):
  ...
  3. Launch via `scripts/launch_l1_multiseed.py` with --seeds 2025,42,9999.
...
Usage:
    ...
    python scripts/launch_sg1_xauusd_vs_v2_phase2_wf_multiseed.py \
        --config configs/sg1_xauusd_volume_study_v2_phase2_wf_multiseed.yaml \
        --dry_run

    # Full launch across gpuhub-2 5-slot pool
    python scripts/launch_sg1_xauusd_vs_v2_phase2_wf_multiseed.py \
        --config configs/sg1_xauusd_volume_study_v2_phase2_wf_multiseed.yaml \
...
Manifest: writes `results/sg1_xauusd_vs_v2_phase2_wf_<timestamp>/manifest.json`
with per-fold (study_id, seeds, exit_code, wandb_finished_ok) entries for the
post-run analyzer. WandB tags per cell:
  `sg-1`, `XAUUSD`, `wf-stage3`, `volume-study-v2`, `phase2`, `vs-v2-phase2-wf`,
  `fold-<i>`, `study-id-<ts>`.
"""
...
RUNTIME_DIR = PROJECT_ROOT / "configs" / "_sg1_xauusd_vs_v2_phase2_wf_runtime"
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] sg1_xauusd_vs_v2_phase2_wf - %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
log = logging.getLogger("sg1_xauusd_vs_v2_phase2_wf")
...
def write_fold_config(base: dict, fold_idx: int, fold: dict, study_id: str) -> Path:
    ...
    out = RUNTIME_DIR / f"sg1_xauusd_vs_v2_phase2_wf_fold_{fold_idx}.yaml"
...
def launch_cell(cfg_path: Path, fold_idx: int, slots: str | None,
    ...
    prefix = f"sg1-xauusd-vs-v2-phase2-wf-fold{fold_idx}"
...
def main() -> int:
    ...
    p.add_argument("--slots", default=None,
                   help="Cross-GPU slot pool: comma-separated host:gpu pairs. "
                        "Recommended for 8-fold WF: 3 slots on gpuhub-2 (max 5 per "
                        "feedback_gpuhub2_max_5_concurrent_sac).")
    ...
    results_dir = PROJECT_ROOT / "results" / f"sg1_xauusd_vs_v2_phase2_wf_{timestamp}"
    results_dir.mkdir(parents=True, exist_ok=True)

    log.info("=" * 70)
    log.info("SG-1-XAUUSD VS-v2 Phase 2 Stage 3 WF Dispatcher (multiseed N=3)")
    ...
    log.info("Next: python scripts/sg1_xauusd_ensemble_eval.py "
             "--wf_config %s --gates_file configs/sg1_xauusd_volume_study_v2_phase2_ensemble.gates.yaml",
