#!/usr/bin/env python
"""GMGP1-BTC Velotrade Stage 3 WF dispatcher — A_long × 2M, multiseed N=5.

Mirrors `scripts/run_data_window_study.py` cell-based pattern, but cells are
WF folds (not [window × budget] cells). For each of 4 folds:
  ...
  3. Launch via `scripts/launch_l1_multiseed.py` with --seeds 123,456,789,1024,2026
     and the user-supplied --slots pool.
Cells run sequentially; within a cell, seeds fan out across slots.

After this dispatcher completes (all 20 train runs done), run:
  python scripts/sg1_xauusd_ensemble_eval.py --wf_config <wf_config> \
...
Usage:
    ...
    python scripts/launch_btc_along_wf_multiseed.py \
        --config configs/gmgp1_btc_along_2m_wf.yaml --dry_run

    # Full launch across 3 GPUs (5-tenant per GPU)
    python scripts/launch_btc_along_wf_multiseed.py \
        --config configs/gmgp1_btc_along_2m_wf.yaml \
        --slots gpuhub-1:0,gpuhub-1:0,gpuhub-1:0,gpuhub-1:0,gpuhub-1:0,\
gpuhub-1:1,gpuhub-1:1,gpuhub-1:1,gpuhub-1:1,gpuhub-1:1,\
gpuhub-2:0,gpuhub-2:0,gpuhub-2:0,gpuhub-2:0,gpuhub-2:0

    # Subset (e.g., only fold 0 for smoke)
    python scripts/launch_btc_along_wf_multiseed.py \
        --config configs/gmgp1_btc_along_2m_wf.yaml --folds 0 --slots ...

Manifest: writes `results/gmgp1_btc_along_wf_<timestamp>/manifest.json` with
per-fold (study_id, seeds, exit_code, wandb_finished_ok) entries for the
post-run analyzer. WandB tags per cell:
  `gmgp1-btc`, `wf-stage3`, `along-2m`, `fold-<i>`, `study-id-<ts>`.
"""
...
RUNTIME_DIR = PROJECT_ROOT / "configs" / "_btc_along_wf_runtime"
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] btc_along_wf - %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
)
log = logging.getLogger("btc_along_wf")
...
def write_fold_config(base: dict, fold_idx: int, fold: dict, study_id: str) -> Path:
    ...
    out = RUNTIME_DIR / f"gmgp1_btc_along_wf_fold_{fold_idx}.yaml"
...
def main() -> int:
    ...
    splitter = RollingWindowSplitter(
        train_months=spl.get("train_months", 22),
    ...
    results_dir = PROJECT_ROOT / "results" / f"gmgp1_btc_along_wf_{timestamp}"
    results_dir.mkdir(parents=True, exist_ok=True)

    log.info("=" * 70)
    log.info("GMGP1-BTC Stage 3 WF Dispatcher — A_long × 2M (multiseed N=5)")
    ...
    # `run_data_window_study.py`'s `wait_runs_finished` (S521).
    ...
    log.info("Next: python scripts/sg1_xauusd_ensemble_eval.py "
             "--wf_config %s --gates_file configs/gmgp1_btc_velotrade_ensemble.gates.yaml",
