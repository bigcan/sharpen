"""Null calibration for the US-EQUITY substrate at H=2 (pre-mine gate, S553-cont-152).

The mandatory pre-mine check, applied to every substrate: score ZERO-ALPHA candidates through the
SHIPPED corrected contract against the REAL base book, and refuse to mine if the base book
manufactures uplift. On the intraday panel this caught a comparator bleeding 30.6%/yr in friction
that let null candidates clear the FULL gate 15.3% of the time (nominal ~1%) purely by diluting the
bleed — a false-positive factory that no amount of downstream rigour would have caught.

Here the base is the validated ETF linear core ({tsmom, rates_carry}) held at `base_hold_horizon: 21`
and joined onto the equity clock, measured at book SR +0.519 over this window — a WINNING comparator,
so the dilution channel should be absent. "Should be" is exactly what the intraday panel looked like
before it was measured, which is why this runs anyway.

DIFFERENCE FROM THE OTHER SUBSTRATES' CALIBRATIONS: this one drives the exposure leg too. The
us_equity corrected-contract gates turn `guards.max_market_beta` on at 0.30 and `market_returns=` is
supplied, so the contract exercised here is byte-for-byte the contract the mine will run under. A
calibration that skipped the live leg would be calibrating a different gate than the one deciding.

PRE-REGISTERED READING (unchanged):
...
Usage:
    python scripts/research/crucible_us_equity_null_calibration.py [--n-null 150]
"""
...
def _null_candidate(panel, rng, *, hold_horizon, cost_bps, ls_min_names, fwd):
    ...
    Random-walk (not iid) scores on purpose: an iid score reshuffles the whole cross-section every
    bar and pays maximal turnover, which would flatter the gate by making every null a big loser.
    A random walk turns over at a realistic rate, so these nulls cost what a real candidate costs.
    """
    ...
    return np.asarray(_book_from_target_weights(
        w, fwd, hold_horizon=hold_horizon, cost_bps=cost_bps))
...
def main() -> int:
    ...
    ap.add_argument("--out",
                    default="results/signal_eval/crucible_equity_breadth/null_calibration.json")
    args = ap.parse_args()
    ...
    print(f"candidate hold {ek['hold_horizon']}, t_min {cc.t_min}, "
          f"uplift_min {cfg.min_combination_uplift}, max_market_beta {cc.max_market_beta}\n")

    rows = []
    rng = np.random.default_rng(20260809)
    for i in range(args.n_null):
        cand = _null_candidate(panel, rng, hold_horizon=ek["hold_horizon"],
                               cost_bps=ek["cost_bps"], ls_min_names=ek["ls_min_names"], fwd=fwd)
        ...
        rows.append({"standalone_sr": float(_ann_sharpe(fin, ppy)),
                     ...
                     "market_beta": float(res.market_beta),
                     ...
                     "exposure_pass": bool(res.exposure_pass)})
        if (i + 1) % 25 == 0:
    ...
    print(f"  standalone SR median {np.median([r['standalone_sr'] for r in rows]):+.4f}"
          f"   (a bleeding comparator is what manufactures uplift — this is the candidate's own)")
    print(f"  marginal dSR  median {np.median([r['delta_sr'] for r in rows]):+.4f}")
    print(f"  market beta   median {np.median([r['market_beta'] for r in rows]):+.4f}")
    print(f"  FULL-GATE PASS RATE {pr:.1%}   (nominal ~1%)")
    for k, v in legs.items():
        print(f"      {k:<20}{v:.1%}")
    verdict = "MINE PERMITTED" if pr <= 0.02 else "DO NOT MINE — base manufactures uplift"
    print(f"\n=> {verdict}")
    ...
    out.write_text(json.dumps({"n_null": n, "pass_rate": pr, "leg_pass_rates": legs,
                               ...
                               "max_market_beta": cc.max_market_beta,
                               "rows": rows}, indent=2), encoding="utf-8")
    print(f"wrote {out}")
    return 0 if pr <= 0.02 else 1
