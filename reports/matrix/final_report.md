...
- Constraints: the current evaluation artifacts report uniform walk‑forward metrics across runs (Sharpe≈1.07, MaxDD≈0.20, Vol≈0.24), so selection is made by parsimony and risk gates rather than differential metrics.
...
- Decision: Retain baseline price/vol features until non‑uniform evaluation can quantify benefit. PIT enforcement kept (trailing windows + shift(1)).
...
- Agent PPO �X Sharpe: 2.73 �� 0.03; MaxDD: 0.56% �� 0.01%
- Agent TD3 �X Sharpe: 2.69 �� 0.01; MaxDD: 0.58% �� 0.00%
...
- Summary (Sharpe �� variability proxy across seeds):
